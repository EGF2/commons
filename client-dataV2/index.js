...
function newClient(url, mode) {
        ...
        for (let i = startTimeout; waitTime <= maxTimeout; i += deltaInterval) {
            ...
            try {
                ...
                if (ignoreErrors) {
                    ...
                    if ((e.body && e.body.code === "ObjectDeletedError") || e.message.includes("Couldn't change deleted object")) return { message: new Date().toISOString() };
    ...
    return {
         ...
         * Get aggregate
        */
        getAggregate: (id, aggregate, options, author) => handle("GET", `/v2/client-data/aggregate/${id}?aggregate=${aggregate}`, null, author),
         ...
         * Get all edges
         */
        getAllEdges: (srcID, edgeName) => handle("GET", `/v2/client-data/getAllEdges/${srcID}/${edgeName}`),
