...
const getOffsetsInfo = consumer =>
  new Promise((resolve, reject) => {
    let assignments = consumer.assignments();
    consumer.committed(
      [...currentPartitions, ...assignments],
      3000,
      (e, data) => {
    ...
    while (true) {
      ...
      if (data.length) {
        ...
        try {
          ...
          const res = await Produser.sendEvent({
            kafkaInfo: message,
...
const newConsumer = async (config, eventHandler, errorHandler) => {
  ...
  if (argv.p && argv.p !== 0)
    debugPartitions = `${argv.p}`.split(",").map(p => {
      // check debug partition
      if (isNaN(Number(p)) || p === "")
        throw new Error(
          `Invalid partition value ${argv.p}. Partition must be number or list of numbers (example: 1 or 1,2,3).`
        );
      ...
      rebalance_cb: async function(err, assignment) {
        if (err.code === Kafka.CODES.ERRORS.ERR__ASSIGN_PARTITIONS) {
          ...
          if (debugPartitions) {
            console.log(
              "\x1b[31m",
              `You are trying to manually subscribe to partitions ${process.env.debugP}. If you do this, the service on Amazon will not stop reading this partition and this can lead to unexpected consequences.`.toUpperCase(),
              "\x1b[0m"
            );
          ...
          this.assign(result);
          Log.info("Rebalance called. Results", {
